...
import {updateUserStart, updateUserSuccess, updateUserFailure, deletesUserFailure, deleteUserStart, deleteUserSuccess, signOutUserStart, signOutUserSuccess} from '../redux/user/userSlice';
...
export default function Profile() {
  ...
  const handleFileUpload=(file)=>{
   ...
   const handleDeleteUser = async ()=>{
    try {
      ...
      const data = await res.json();
      if (data.success === false) {
        dispatch(deletesUserFailure(data.message));
        return;
      }
      dispatch(deleteUserSuccess(data));
      
    } catch (error) {
      dispatch(deletesUserFailure(error.message));
  ...
  return (
    <div className="p-3 max-w-lg mx-auto">
      ...
      <Link className="bg-green-700 text-white p-3 rounded-lg upperclass text-center hover: opacity-95" to={"/create-listing"}>
        Create Listing
      </Link>
      ...
      <div className="flex flex-col gap-4">
        <h1 className="text-center mt-7 text-2xl text-semibold ">Your Listing </h1>
